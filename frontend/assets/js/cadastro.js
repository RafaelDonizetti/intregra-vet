...
btnCadastro.addEventListener("click", function (e) {
  ...
  if (!email || !emailConfirm) {
    showModal("Preencha ambos os campos de e-mail.");
    return;
  } else if (email !== emailConfirm) {
    showModal("Os E-mails não estão iguais.");
    return;
  } else if (!email.match(emailRegex)) {
    showModal("Você não digitou um e-mail valido");
    return;
  } else {
    if (!password || !passwordConfirm) {
      showModal("Preencha ambos os campos de senha.");
      return;
    } else if (password !== passwordConfirm) {
      showModal("As senhas não estão iguais.");
      return;
    }
  }
